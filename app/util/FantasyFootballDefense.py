...
"""
Method to return a fantasy point calculation based on past defensive career stats
"""


def calc_past_defensive_fantasy_value(career, year):
	result = 0
	for index, row in career.iterrows():
		...
		if (8 <= row.date.month <= 12 and row.date.year == year) or \
			...
			points_from_pass_yds = \
				calc_defensive_fantasy_value_by_yards(row[ffc.STATS_KEY_OFF_PASS_YDS], ffc.STATS_VAL_OFF_PASS_YDS)
			result = result + points_from_pass_yds
			points_from_pass_tds = \
				calc_defensive_fantasy_value_by_tds(row[ffc.STATS_KEY_OFF_PASS_TD], ffc.STATS_VAL_OFF_PASS_TD)
			result = result + points_from_pass_tds
			points_from_pass_ints = \
				calc_defensive_fantasy_value_by_ints(row[ffc.STATS_KEY_OFF_PASS_INT], ffc.STATS_VAL_OFF_PASS_INT)
			result = result + points_from_pass_ints

			# points from Rushing
			points_from_rush_yds = \
				calc_defensive_fantasy_value_by_yards(row[ffc.STATS_KEY_OFF_RUSH_YDS], ffc.STATS_VAL_OFF_RUSH_YDS)
			result = result + points_from_rush_yds
			points_from_rush_tds = \
				calc_defensive_fantasy_value_by_tds(row[ffc.STATS_KEY_OFF_RUSH_TD], ffc.STATS_VAL_OFF_RUSH_TD)
			result = result + points_from_rush_tds

			# points from Receiving
			points_from_recv_yds = \
				calc_defensive_fantasy_value_by_yards(row[ffc.STATS_KEY_OFF_RECV_YDS], ffc.STATS_VAL_OFF_RECV_YDS)
			result = result + points_from_recv_yds
			points_from_recv_tds = \
				calc_defensive_fantasy_value_by_tds(row[ffc.STATS_KEY_OFF_RECV_TD], ffc.STATS_VAL_OFF_RECV_TD)
			result = result + points_from_recv_tds

			# points from Kick Returns
			points_from_kick_return_yds = \
				calc_defensive_fantasy_value_by_yards(row[ffc.STATS_KEY_OFF_RTRN_YDS], ffc.STATS_VAL_OFF_RTRN_YDS)
			result = result + points_from_kick_return_yds
			points_from_kick_return_tds = \
				calc_defensive_fantasy_value_by_tds(row[ffc.STATS_KEY_OFF_RTRN_TD], ffc.STATS_VAL_OFF_RTRN_TD)
			result = result + points_from_kick_return_tds
	return result


"""
Method to return fantasy point calculations for a player's entire defensive career.
A career is a list of games.  This function returns a numpy array with an element
for every season in the player's career.
"""


def calc_past_defensive_fantasy_values(career):
	seasons = list()
	first_year = -1
	last_year = -1
	for index, row in career.iterrows():
	...
	for year in range(first_year, last_year + 1, 1):
		fantasy_value_for_year = calc_past_defensive_fantasy_value(career, year)
...
def calc_defensive_fantasy_value_by_yards(yards, chart):
	result = 0
	rules = chart.split(",")
...
"""
Method to return a fantasy point calculation based on any defensive TD.
By default 6 pts are awarded per TD no matter how the TD is scored.
"""


def calc_defensive_fantasy_value_by_tds(tds, td_value):
	return tds * td_value
...
def calc_defensive_fantasy_value_by_ints(ints, int_value):
